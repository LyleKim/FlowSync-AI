...
문제 : 팀원들이 남긴 기록을 바탕으로 AI를 활용하여 앞으로 내가 진행해야 할 일을 추천받을 때,  
AI 응답이 늦을 경우가 있다.(3~7초) 사용자 측에서 불편함.
  
해결1 : POST 메서드로 받고 AI 응답을 바로 프론트로 리턴하고,  
생성된 작업 리스트는 프론트에서 다시 응답을 받을 때 비로소 PATCH로 저장한다.  
...
문제 : "작업 상세 정보"를 받을 때, 내부의 팀원들이 해당 작업에 관한 진행 상황을 등록할 때,  
일반적으로 POST를 사용할 것인가, 아니면 "작업 상세 정보" 페이지와 관련된 데이터를 하나의 모델로 묶어서 PATCH로 처리할 것인가?  
...
결과 : 별도 조인 작업이 필요 없어 관리 요소를 줄일 수 있음. 또한 불필요한 view의 메서드를 추가할 필요 없음.

<br>

- 조건부 폴링 데이터 동기화 (서버,네트워크 비용 최적화)  
  
문제 : SPA(Single Page Application) 구조 특성상 브라우저 메모리에 데이터를 들고 있어,
다른 팀원이 추가한 업무 상황이 실시간으로 공유되지 않음.  
그렇다고 무작정 주기적인 전체 데이터 조회를 돌리면 서버 및 네트워크 자원 낭비가 심해짐.  

해결 : 프론트엔드에서 3분 주기로 풀링을 진행. 그러나 백엔드에 부하를 주지 않도록 ETag와 최신 수정 시간을 검증하여,  
