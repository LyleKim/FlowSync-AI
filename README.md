...
해결 : 
- POST 메서드를 사용해서 처리를 한다면 모델을 외래키를 사용해서 분리할 필요가 있다. 
...
문제 : SPA(Single Page Application) 구조 특성상 브라우저 메모리에 데이터를 들고 있어, 다른 팀원이 추가한 업무 상황이 실시간으로 공유되지 않음. 그렇다고 무작정 주기적인 전체 데이터 조회를 돌리면 서버 및 네트워크 자원 낭비가 심해짐.
