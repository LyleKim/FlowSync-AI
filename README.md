...
해결 : "작업 상세 정보" 페이지의 경우에 "관련 역할 및 검토 기록", "작업 체크리스트", "작업 프로세스 흐름도" 3가지 정보를 보여준다.  
**"관련 역할 및 검토 기록"에 팀원이 "진행 상황"을 댓글로 남길 때, HTTP 메서드는 무엇을 쓸 것인가?**  
...
##TDL  
- Claude Code 기반 기록 관리 시스템 제작  
현재 :  
수동으로 현재 나의 작업 사항을 기록해야한다. 일이 바쁘다면 기록은 건너뛰고 다른 연락 수단을 취할 수 있다.  
그러나 그래서는 향후 특정 이슈에 대한 트러블 슈팅 과정을 알 수 없다.  
  
미래 :  
현재(2026년)는 많은 개발자가 Claude Code를 사용하여 개발을 진행하고 있다.  
따라서 Claude Code의 작업 상황을 요약, 필터링해서 FlowSyncAI의 API로 전송하도록 설계한다.  